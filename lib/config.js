...
function profile() {
    ...
    states[4] = "FIREPLACE";
    states[5] = "EXTRA";
    const profile = createState("ACTIVE_PROFILE",
        "indicator",
        states,
...
function bool(objects, key, role, textId, writable = false, channel = null) {
    objects.set(channel ? channel + "." + key : key, new VlxObjectConfig(key, [key], handler.read_bool, create_iobroker_object(key, role, "state", "boolean", null, writable, textId, ""), channel));
}

function number(objects, key, role, textId, unit, writable = false, channel = null) {
    objects.set(channel ? channel + "." + key : key,
        new VlxObjectConfig(
            key,
            [key],
            handler.read_float,
