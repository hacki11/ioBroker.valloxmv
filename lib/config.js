...
function channels() {
    ...
    let vlxChannels = [
        create_iobroker_object("profiles", "", 'channel', '', null, false, 'id_text_web_profiles'),
        create_iobroker_object("profiles.home", "", 'channel', '', null, false, 'id_text_title_home_profile'),
    ]
...
function number(objects, key, role, textId, unit, writable = false) {
    objects.set(key, new VlxObjectConfig(key, [key], nothing, create_iobroker_object(key, role, "state", "number", [], writable, textId, unit)));
}

function state(objects, key, role, states, textId = "", writable = false) {
    objects.set(key, createState(key, role, states, textId, writable));
}
